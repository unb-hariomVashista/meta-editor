...
export const action = async ({ request }) => {
  ...
  const gidIdx = headers.findIndex(h => h.includes("gid"));
  ...
  if (gidIdx === -1 || nsIdx === -1 || keyIdx === -1 || typeIdx === -1 || valIdx === -1) {
    return { error: "CSV is missing required headers (product gid, metafield namespace/names, metafield key, metafield type, metafield value)." };
  }
  ...
  return { success: `Successfully imported ${successCount} metafields.`, errors };
};
    ...
    <div>
      <s-section>
        <div style={{ marginBottom: "20px" }}>
          <h2 style={{ fontSize: "1.5rem", marginBottom: "10px" }}>Import Product Metafields</h2>
          ...
          <code style={{ display: "block", padding: "10px", background: "#f4f6f8", margin: "10px 0", borderRadius: "4px" }}>
            product gid, product handle, product title, metafield names, metafield key, metafield type, metafield value
          </code>
        </div>
